...
for squad in $(jq -r '.squads | to_entries[] | select(.value.zone == "remote-opaque") | .key' "$MESH_JSON"); do
  ...
  if ! is_safe_http_url "$source"; then
    echo "⚠ $squad: invalid source URL (must be http/https)"
    continue
  fi

  if ! is_confined_target_path "$target"; then
    echo "⚠ $squad: invalid sync_to path (must be confined relative path)"
    continue
  fi
  ...
  if [ "$auth" = "bearer" ]; then
    ...
    # Build argv safely; never eval config-derived content.
    [ -n "${!token_var:-}" ] && auth_flag=(--header "Authorization: Bearer ${!token_var}")
  fi

  curl --silent --fail "${auth_flag[@]}" "$source" -o "$target/SUMMARY.md" 2>/dev/null \
