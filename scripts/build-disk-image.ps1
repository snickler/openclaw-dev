...
function Invoke-SecretScan {
    ...
    $pattern = '(AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{20,}|-----BEGIN (RSA|EC|OPENSSH) PRIVATE KEY-----|api[_-]?key\s*[:=]\s*["''][^"'']{12,}|token\s*[:=]\s*["''][^"'']{12,}|client[_-]?secret\s*[:=])'
    if (Test-Path $Path -PathType Container) {
        $matches = Get-ChildItem -Path $Path -Recurse -File | Select-String -Pattern $pattern -CaseSensitive:$false -ErrorAction SilentlyContinue
    } else {
