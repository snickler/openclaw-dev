...
function renderTools(bundle) {
  ...
  return [
    ...
    "- Use `agent_to_agent` for answer-required specialist calls only when that tool is actually exposed in the current tool list; otherwise route to the specialist through the Agents view or direct `openclaw agent --agent <id>` flow. Use `sessions_spawn`/`subagents`/`sessions_yield` only for fire-and-forget background work; use `sessions_history` for transcript recall.",
...
function renderCommands(bundle) {
  return [
    ...
    "- `task` / `runSubagent` → Use `agent_to_agent` only when the current tool list exposes it and the current answer depends on specialist output. If `agent_to_agent` is absent, use the Agents view or direct `openclaw agent --agent <id>` flow for answer-required specialist work. Use `sessions_spawn` + `subagents` + `sessions_yield` only for background work where an accepted child-session handle is enough.",
...
function renderCommandWrappers() {
  ...
  return [
    ...
    "",
    "## Mapping",
    "- `task` / `runSubagent` → use `agent_to_agent` only when it is exposed and the current answer depends on specialist output; if it is absent, use the Agents view or direct `openclaw agent --agent <id>` flow for answer-required specialist work; use `sessions_spawn` + `subagents` + `sessions_yield` only for background work where an accepted child-session handle is enough.",
...
function renderHostedCompatSkill() {
  return [
    ...
    "",
    "## Patterns",
    "- Prefer `agent_to_agent` for specialist consultation/delegation when the current tool list exposes it; if it is absent, use the Agents view or direct `openclaw agent --agent <id>` flow for answer-required specialist work.",
    "- Prefer `sessions_spawn` + `subagents` + `sessions_yield` only for fire-and-forget background or parallel work where the current response does not depend on child output; use `sessions_history` for transcript recall.",
...
function renderCoordinatorAgents(agent, bundle) {
  return [
    ...
    "- Use `agent_to_agent` when it is exposed and the current answer depends on specialist output; if it is absent, route answer-required specialist work through the Agents view or direct `openclaw agent --agent <id>` flow. Use `sessions_spawn` / `subagents` only for fire-and-forget background work when those tools are exposed.",
